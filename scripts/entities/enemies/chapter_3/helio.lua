...
function HELIO:NPCUpdate(npc)
    if npc.Type == BotB.Enums.Entities.HELIO.TYPE and npc.Variant == BotB.Enums.Entities.HELIO.VARIANT then 
        ...
        if npc.State == 0 then
            ...
            data.nyctoButtonPosition = HELIO:FindNearestLightsOnButton(npc.Position)
        end
...
Mod:AddCallback(ModCallbacks.MC_NPC_UPDATE, HELIO.NPCUpdate, Isaac.GetEntityTypeByName("Helio"))

function HELIO:FindNearestLightsOnButton(pos)
    local baseDist = 999999
    local nyctoTarget
    local roomEntities = Isaac.GetRoomEntities() -- table
    for i = 1, #roomEntities do
        local entity = roomEntities[i]
        if entity.Type == EntityType.ENTITY_GENERIC_PROP and (entity.Variant == BotB.Enums.Props.POWER_ON_BUTTON.VARIANT or entity.Variant == BotB.Enums.Props.LIGHT_SWITCH.VARIANT) then
...
function HELIO:changeAllLightSwitches()
    local roomEntities = Isaac.GetRoomEntities() -- table
    for i = 1, #roomEntities do
        local entity = roomEntities[i]
