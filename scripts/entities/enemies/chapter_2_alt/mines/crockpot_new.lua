...
function CROCKPOT_NEW:NPCUpdate(npc)
    ...
    if npc.Type == BotB.Enums.Entities.CROCKPOT_NEW.TYPE and npc.Variant == BotB.Enums.Entities.CROCKPOT_NEW.VARIANT then 
	    ...
	    npc.Velocity = Vector.Zero
            ...
            data.crockPotIsDeactivated = false
        ...
        if npc.State ~= 16 or data.AlwaysOn then
            ...
            if npc.State == 8 then
                data.crockPotIsDeactivated = true
            end
            ...
            if npc.State == 4 then
                ...
                if sprite:IsEventTriggered("Back") then
                    ...
                    data.crockDot.CollisionDamage = 0
        ...
        elseif npc.State == 16 then
            if data.AlwaysOn then
                npc.State = 3
                sprite:Play("Idle")
            else
                if not sprite:IsPlaying("Deactivate") and data.crockPotIsDeactivated == false then
                    sprite:Play("Deactivate", true)
                    data.crockPotIsDeactivated = true
                end
                
                if sprite:IsFinished("Deactivate") then
                    sprite:Play("Inactive", true)
                end
            end
            --print("balls")
...
Mod:AddCallback(ModCallbacks.MC_NPC_UPDATE, CROCKPOT_NEW.NPCUpdate, Isaac.GetEntityTypeByName("Crockpot (New)"))