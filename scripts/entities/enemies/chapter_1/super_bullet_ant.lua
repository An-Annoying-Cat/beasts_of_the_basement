local Mod = BotB
local SUPER_BULLET_ANT = {}
local Entities = BotB.Enums.Entities

--print(Isaac.GetSoundIdByName("PlusTen"))

function SUPER_BULLET_ANT:NPCUpdate(npc)


    if npc.Type == BotB.Enums.Entities.SUPER_BULLET_ANT.TYPE and npc.Variant == BotB.Enums.Entities.SUPER_BULLET_ANT.VARIANT then 
        ...
        if npc.State == 99 then
            ...
            if data.botbAntWalkCooldown ~= 0 then
                ...
                if targetdistance <= 200 then
                    ...
                    local bullet = Isaac.Spawn(9, 0, 0, npc.Position, Vector(12,0):Rotated(targetangle-10), npc):ToProjectile()
...
Mod:AddCallback(ModCallbacks.MC_NPC_UPDATE, SUPER_BULLET_ANT.NPCUpdate, Isaac.GetEntityTypeByName("Super Bullet Ant"))
...
--[[
function SUPER_BULLET_ANT:BulletCheck(bullet)

    --Seducer projectiles spawn red creep when they splat
    if bullet.Parent ~= nil and bullet.Parent.Type == BotB.Enums.Entities.SUPER_BULLET_ANT.TYPE and bullet.Parent.Variant == BotB.Enums.Entities.SUPER_BULLET_ANT.VARIANT then
...
Mod:AddCallback(ModCallbacks.MC_POST_PROJECTILE_UPDATE, SUPER_BULLET_ANT.BulletCheck)]]