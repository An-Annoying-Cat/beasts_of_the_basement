...
if EID then
	EID:addCollectible(Isaac.GetItemIdByName("Houseplant"), "+1 max health. #When hurt, leave a trail of red creep for 1/3rd of a second.#{{Warning}}Amount of time which creep is left per player hurt increases linearly with the amount of copies of this item that the player possesses.")
end
	...
	function HOUSEPLANT:houseplantHurt()
		...
		local data = player:GetData()
			if player:HasCollectible(Isaac.GetItemIdByName("Houseplant")) then
				data.houseplantTimer = data.houseplantTimer + (20 * player:GetCollectibleNum(BotB.Enums.Items.HOUSEPLANT, false))
			end
		end
