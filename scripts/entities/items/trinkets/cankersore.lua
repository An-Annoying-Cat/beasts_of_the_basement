...
if EID then
	EID:addTrinket(Isaac.GetTrinketIdByName("A Single Raisin"), "Grants a fading boost to fire rate after the player successfully picks up any kind of heart, accompanied by fake damage.")
end
...
function CANKER_SORE:getHeart(pickup,collider,_)
    ...
    if collider.Type == Isaac.GetEntityTypeByName("Player") then
        ...
        if (pickup.SubType == HeartSubType.HEART_FULL or pickup.SubType == HeartSubType.HEART_HALF or pickup.SubType == HeartSubType.HEART_DOUBLEPACK) and collider:ToPlayer():CanPickRedHearts() then
			if data.cankerSoreTimer ~= nil then
				data.cankerSoreTimer = data.cankerSoreTimer + 240
				player:UseActiveItem(CollectibleType.COLLECTIBLE_DULL_RAZOR, UseFlag.USE_MIMIC)
			end
