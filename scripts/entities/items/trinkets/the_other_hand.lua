local Mod = BotB
local THE_OTHER_HAND = {}
local ff = FiendFolio

if EID then
	EID:addTrinket(Isaac.GetTrinketIdByName("The Other Hand"), "All chests are rerolled on spawn into a random type. #{{Warning}}This reroll is completely unweighted!")
end
...
--[[
function THE_OTHER_HAND:doBlankWhiteCardReroll()
  ...
  Mod:AddCallback(ModCallbacks.MC_POST_NEW_ROOM, THE_OTHER_HAND.doBlankWhiteCardReroll, 0)
]]
...
local grng = RNG()
function THE_OTHER_HAND:pickupUpdate(pickup)
    local doTheyHavePandemonium = false
    local players = getPlayers()
    for i=1,#players do
        local player = players[i]
        if players[i]:ToPlayer():HasTrinket(BotB.Enums.Trinkets.THE_OTHER_HAND, false) then
	...
	if (pickup:GetSprite():IsPlaying("Appear") or pickup:GetSprite():IsPlaying("AppearFast")) and pickup:GetData().noPandemoniumMorph ~= true and pickup.FrameCount == 2 then
        ...
        if rerollType == "" or rerollType ~="Chests" then return end
...
Mod:AddCallback(ModCallbacks.MC_POST_PICKUP_UPDATE, THE_OTHER_HAND.pickupUpdate)